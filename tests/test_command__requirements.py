...
class RequirementsTestCase(TestCase):
    ...
    def test_freeze_unfreeze(self):
        ...
        with self.temporary_directory_context():
            ...
            self.projector("requirements add infi.pyutils --commit-changes")
            ...
            self.assertIn("infi.pyutils>=", open("buildout.cfg").read())
            with self.assert_new_commit():
            ...
            self.assertIn("infi.execute<=0.0.7", open("buildout.cfg").read())
            self.assertNotIn("infi.pyutils>=", open("buildout.cfg").read())
            self.assertIn("infi.pyutils", open("buildout.cfg").read())
